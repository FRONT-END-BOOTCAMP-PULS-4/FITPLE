...
//         const { title, content, duration, workMode, status, positionIds, skillIds } = body;

//         // 필수 항목 체크 (타이틀, 콘텐츠, 기간, 근무모드, 상태만 확인하고, 나머지 항목은 없으면 건너뛰기)
//         if (!title || !content || !duration || !workMode || !status || !positionIds || !skillIds) {
//             return NextResponse.json({ error: '필수 항목을 만족하지 못했습니다.' }, { status: 400 });
//         }
...
//         const { title, content, duration, workMode, status } = body;
